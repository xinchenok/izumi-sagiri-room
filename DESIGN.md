...
控件状态使用 0.2s，房间镜头与人物容器变化使用 0.65s，缓动为 `cubic-bezier(0.16, 1, 0.3, 1)`；观察器按钮为 0.15s。活泼状态的人物呼吸周期为 5.5s、位移仅 1.5px，窗雨为 2.8s 线性循环，纸面偷看为 0.7s 的裁切变化。它们分别来自呼吸、窗雨和藏画行为，不增加与角色无关的装饰动画。
