...
这是一个 code-led 的房门偷看叙事：访客先站在银蓝走廊里，被半掩的可可木门和门缝后的蓝眼睛拦住；轻轻敲门、获得回应之后，才依次进入画桌、雨夜共同创作、衣橱相册、胶片画稿与晚安纸条。它不是角色资料卡集合，而是一段有距离、有许可、会回应、会留下具体共同经历的拜访过程。
...
**叙事顺序规则。** 响应式只能重排空间，不能改变“门口—画桌—一起画—衣橱—画稿—晚安”的阅读顺序，也不能把人物证明图折叠到交互之后。
...
敲门使用“受惊—脸红—开门”，共同创作使用短帧淡入、自然眨眼、雨线与笔尖微光，换装使用 520ms 翻页，画稿使用淡出与轻微放大，发现秘密使用猫爪进度与纸屑，反馈表情使用 420ms 轻弹。状态持久化保存服装、已发现秘密、有效回访次数与时间、共同完成的画、画册位置、音量和只看字幕偏好；图片失败保留门、稿纸、文字和可继续选择的原生控件，语音失败保留双语字幕。

在 `prefers-reduced-motion: reduce` 下，平滑滚动关闭，动画和过渡压缩到 0.01ms，纸屑、共同创作雨线与笔尖微光隐藏，随机眨眼停止，视差不注册，门板不旋转而以低透明度露出内景；脚本中的门、共同创作、换装和画稿等待同步缩短。减少动态只改变表现，不删除内容、状态、字幕或反馈。
...
`assets/v3` 保留原 PNG 维护源与五秘密画桌；运行时人物表情和五套换装使用 `assets/v5` 的 1122 × 1402 高质量 WebP，并为窄屏提供 560 × 700 版本。两套新增换装同时保留生成源 PNG，完整提示词与导出参数见 `docs/outfit-v5-image-prompts.md`。十一张运行时画册图及对应缩略图使用 `assets/v4` 的 4:3 WebP。共同创作使用 `assets/v6` 的四组 1448 × 1086 / 720 × 540 人物帧和三张 960 × 720 小猫成稿，生成方向、导出规格与 SHA-256 见 `docs/drawing-v6-image-prompts.md`。二十二句日语回答使用 `assets/audio/v4` 下的 MP3，每句对应的完整 AivisSpeech 查询参数保存在 `assets/audio/v4/queries`。页脚必须长期保留 AivisSpeech「天深シノ」、依据 ACML-NC 1.0 使用及模型来源链接的署名，也必须保留 ZCOOL KuaiLe 依据 SIL Open Font License 1.1 使用的说明。不得改为远程热链，也不得使用官方截图、动画音轨或声优声纹克隆。
