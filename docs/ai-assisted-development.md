...
- 当前用户可见功能基线：[`5cf713a`](https://github.com/xinchenok/izumi-sagiri-room/commit/5cf713a9e08edc720e8463e0011de76acee39257)
- 当前页面缓存标识：`20260813-1`
...
1. 把页面整理为“门口初见 → 房间探索 → 雨夜一起画 → 日常换装 → 秘密画稿 → 晚安告别”的房门偷看叙事。
2. 建立草莓粉、银蓝、薄荷青、可可棕和奶油纸色组成的视觉系统，并记录在 `DESIGN.md`。
3. 生成一张角色身份锚点，再基于同一身份制作四张首屏表情、五张换装图、十一张运行时画册情景图、一张五秘密画桌场景图，以及四张雨夜共同创作人物帧和三张小猫成稿。
4. 使用原生 HTML、CSS 和 JavaScript 实现敲门、三阶段回访文案、表情切换、无常驻标记彩蛋、雨夜共同创作、换装相册、章节画廊与暗房、语音字幕和控制、晚安纸条和统一反应角。
5. 将人物表情、服装、台词、反应、画廊和彩蛋集中在 `script.js` 的 `CONTENT` 对象中，避免内容散落在事件处理代码里。
6. 在 `sagiri-room-state-v2` 中扩展回访次数、上次有效拜访、共同完成的画、画册位置、音量和只看字幕偏好，并保留从 `sagiri-room-state-v1` 迁移服装和彩蛋记录的逻辑。
7. 使用 AivisSpeech 在本地生成二十二句不同回答；十八句轻声、普通或开心回答按入口洗牌并保证一轮听完前不重复，四句 `ふきげん` 回答只在访客短时间连续催门或催稿时使用，同时展示当前回答的日文和简体中文字幕。
8. 将运行时人物与换装切换为完整尺寸 / 560px 两档高质量 WebP，共同创作舞台使用 1448 × 1086 / 720 × 540 两档 WebP，将 1.5 MB 标题字体缩成当前页面字符子集 WOFF2，并取消画册与衣橱的一次性全量预取。
...
本节是项目截至 2026-08-13 的交付账本。它记录已经进入 GitHub `main` 并由 GitHub Pages 提供的实际工作；未完成的规划仍只放在“已知问题与后续规划”，不会混写成已交付功能。
...
- 将约 1.5 MB 的标题 TTF 缩成当前页面字符子集 WOFF2；V6 重建子集后新增 58 个字体源可提供的页面字形，覆盖当前简体中文文案，运行时字体约 89 KB；日语字幕继续优先使用系统 CJK 字体。画册和衣橱由全量预取改为仅预取当前相邻内容。
- 在同一 390px 冷缓存条件下，页面请求量从约 10.79 MB 降到约 1.04 MB，减少约 90.4%；运行时不热链第三方图片、字体或音频。
- 将持久状态集中到 `sagiri-room-state-v2`，保存服装、五秘密、回访、共同完成的画、画册位置、音量和只看字幕偏好，并保留从 v1 迁移服装与秘密记录的兼容逻辑。
...
- 分别保存五秘密画桌、八张新增害羞画册、两套新增换装和 V6 雨夜共同创作的提示词、参考素材、导出规格与校验值，方便后续在不破坏角色一致性的前提下维护。
...
- 本次知识收尾开始前，本地功能分支与远端 `main` 的完整 Git tree 一致，156 个受版本控制文件全部存在且非空，没有漏传或未提交的运行时代码、图片、字体、音频、查询 JSON 和提示词文档。
- 正式网址返回 `HTTP 200`，线上 HTML 使用 `styles.css?v=20260813-1` 与 `script.js?v=20260813-1`；线上 HTML、CSS 和 JavaScript 与功能基线逐字节一致。
...
- 静态门禁包括 `node --check script.js`、全部本地资源引用、图片和音频非空、语音清单哈希与时长、UTF-8 无 BOM、乱码特征、键盘操作和缓存标识。文档更新后仍需重复执行适用门禁，并以新的远端 `main` 作为最终发布证据。
...
旧版 PNG 保留在仓库中作为维护源；首屏与换装运行时使用 `assets/v5/`，画册使用 `assets/v4/`，五秘密画桌继续优先使用 `assets/v3/desk-secrets.webp`，共同创作使用 `assets/v6/`。新增画册、换装和共同创作的完整提示词、校验值与导出方法分别见 `docs/gallery-v4-image-prompts.md`、`docs/outfit-v5-image-prompts.md` 和 `docs/drawing-v6-image-prompts.md`。
