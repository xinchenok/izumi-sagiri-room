...
本项目没有收集训练数据，没有训练、微调或克隆任何声优声纹，也没有截取动画音轨。语音部分采用现成且许可范围明确的 AivisSpeech 预训练模型“天深シノ”，在本地通过 AivisSpeech Engine 将四句日语文本合成为 WAV 文件；每句另由原始 WAV 制作两种不改变台词的轻度后期语气版本。
...
3. 生成一张角色身份锚点，再基于同一身份制作四张首屏表情、三张换装图、三张画廊情景图和一张五秘密画桌场景图。
...
7. 使用 AivisSpeech 在本地生成四段日语 WAV，再制作八段轻度后期 MP3；每个入口从三种语气中随机选择且不连续重复，同时始终展示完全一致的日文台词和简体中文字幕。
...
| `assets/v3/` | 当前页面使用的十一张 PNG 角色插画；五秘密画桌另有 WebP 优先资源 |
| `assets/audio/v3/` | 四段原始 WAV 日语台词与八段轻度后期 MP3 语气版本 |
...
当前四个原始文件为 44.1 kHz、单声道 WAV。网页通过用户点击后创建 `Audio` 实例播放，从该场景的原始 WAV 和两段后期 MP3 中随机选择，并避免连续重复同一版本；开始新台词前会停止上一段。播放失败时仍保留日文台词、简体中文字幕和视觉反馈，且不会回退到系统 TTS。
...
- 不在仓库中重新分发“天深シノ”模型包；仓库只保存页面需要的最终 WAV 与其轻度后期 MP3。
- 当前语音模型采用 ACML-NC 1.0，网站必须维持许可允许的非商业使用范围并保留来源说明。
- 若未来出现商业化、广告、付费下载或品牌合作，必须先停用现有语音并重新完成角色版权、模型许可和字体许可审查。

## 验证与上线方式

2026-08-11 本轮修复已完成 JavaScript 语法、CSS 解析、27 个本地资源引用、音频解码、UTF-8、热点几何和 DOM 交互检查；五个热点在最窄画桌宽度下仍大于 44px、互不重叠，十二种语气可轮换且不会连续重复，音频失败时字幕仍保留。受控开发容器禁止 Chromium 创建系统套接字，因此本轮没有在本地重新生成多视口截图；发布前仍须在可启动浏览器的环境中复核下列完整清单：

- 1440×900、1024×768 和 390×844 三种视口的布局与横向溢出检查。
- 门板键盘操作、三套换装、六张画廊内容、五个彩蛋、隐藏留言和 v1 状态迁移。
- 四段台词、十二种语气的随机轮换、连续播放、跨场景打断和缺失文件降级。
