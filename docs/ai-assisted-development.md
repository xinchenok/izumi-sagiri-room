...
- 当前用户可见功能基线：[`d93b4bc`](https://github.com/xinchenok/izumi-sagiri-room/commit/d93b4bc78c6749cd075cce8d6296240efbabc055)
- V7 发布候选缓存标识：`20260813-2`
...
6. 在 `sagiri-room-state-v2` 中扩展回访次数、上次有效拜访、共同完成的画、连续房间位置与天气、房间声开关、画册位置、音量和只看字幕偏好，并保留从 `sagiri-room-state-v1` 迁移服装和彩蛋记录的逻辑。
7. 使用 AivisSpeech 在本地生成二十二句不同回答；十八句轻声、普通或开心回答按入口洗牌并保证一轮听完前不重复，四句 `ふきげん` 回答只在访客短时间连续催门或催稿时使用，同时展示当前回答的日文和简体中文字幕。
8. 将运行时人物与换装切换为完整尺寸 / 560px 两档高质量 WebP，共同创作与 V7 衣橱场景使用 1448 × 1086 / 720 × 540 两档 WebP，将 1.5 MB 标题字体缩成当前页面字符子集 WOFF2，并取消画册、衣橱与连续房间的一次性全量预取。
9. 建立 2048、1440、1024 和 390px 的发布前浏览器清单，并对键盘操作、手机横滑、房间自主变化、暗房、回访状态、只看字幕、独立拟音开关、减少动态效果、音频失败降级、资源引用和 UTF-8 编码进行自动检查。
...
本节是项目截至 2026-08-13 的交付账本。带提交链接的发布表记录已经进入 GitHub `main` 并由 GitHub Pages 提供的实际工作；单独标明“发布候选”的 V7 条目只代表本地已完成并通过门禁，直到补入远端提交与正式页面核验前都不算线上交付。未完成的规划仍只放在“已知问题与后续规划”，不会混写成已交付功能。
...
- 增加音量、停止和“只看字幕”控制，并保存音量与静音偏好；音频加载失败时继续显示完整字幕与视觉反应，不调用系统 TTS，也不自动播放声音。
...
- 将约 1.5 MB 的标题 TTF 缩成当前页面字符子集 WOFF2；V7 最终契约同步后再次重建，原字体能提供的 790 个当前页面源码非 ASCII 字符全部进入子集，运行时字体约 93 KB；原字体不包含的日语假名与日文字形继续优先使用系统 CJK 字体。画册、衣橱和连续房间只预取当前相邻或即将使用的内容。
- 在同一 390px 冷缓存条件下，页面请求量从约 10.79 MB 降到约 1.04 MB，减少约 90.4%；运行时不热链第三方图片、字体或音频。
- 将持久状态集中到 `sagiri-room-state-v2`，保存服装、五秘密、回访、共同完成的画、连续房间位置与天气、房间声开关、画册位置、音量和只看字幕偏好，并保留从 v1 迁移服装与秘密记录的兼容逻辑。
...
- 分别保存五秘密画桌、八张新增害羞画册、两套新增换装、V6 雨夜共同创作和 V7 连续房间的提示词、参考素材、拟音方法、导出规格与校验值，方便后续在不破坏角色一致性的前提下维护。
- 建立 `AGENTS.md`，固定零框架架构、本地资源、UTF-8、集中内容对象、最低验证、文档同步和“完成后默认提交、推送、合并、上线并在线复核”的协作规则。
- 在页脚和 README 保留 AivisSpeech“天深シノ”模型、ACML-NC 1.0、站酷快乐体与 SIL Open Font License 1.1 的来源说明，并明确页面不代表版权方官方立场。
...
- V7 发布候选在 1440 × 1000、1024 × 900 与 390 × 844 完成四地点切换、四地点重复小事件、约九秒自主变化、桌边成稿陈列、方向键、手机横滑、房间天气、独立拟音开关和刷新持久化检查；三档均无横向溢出、404、控制台或脚本错误。旧 V6 状态缺失三个新字段时安全回落；390px 减少动态模式确认雨线与四类物件动作隐藏、过渡为 0.01ms。该项目前是本地发布候选证据，最终 `live verified` 必须以远端 `main` 和正式 Pages 逐字节核验补齐。
- V7 发布候选共有 178 个版本控制内或待加入版本控制的文件，全部存在且非空；静态门禁包括 `node --check script.js`、CSS 语法、全部本地资源引用、93 张 PNG / WebP 解码、42 段 MP3 / WAV 解码、二十二句语音清单哈希与时长、七项 V7 新资源哈希、标题字体字符覆盖、UTF-8 无 BOM、乱码特征、Markdown 内链、键盘操作和 `20260813-2` 缓存标识。最终仍需以新的远端 `main` 和正式 Pages 作为发布证据。
...
| `assets/audio/v7/` | 笔尖、布料、衣架、窗帘 / 雨声四段操作触发的本地短拟音 |
