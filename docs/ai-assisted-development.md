...
本项目没有收集训练数据，没有训练、微调或克隆任何声优声纹，也没有截取动画音轨。当前语音采用许可范围明确的 AivisSpeech 预训练模型“天深シノ”，在本地通过 AivisSpeech Engine 独立合成十六句日语回答；页面没有把同一句台词变速、变调后当成新回答。
...
3. 生成一张角色身份锚点，再基于同一身份制作四张首屏表情、三张换装图、七张运行时画册情景图和一张五秘密画桌场景图。
...
7. 使用 AivisSpeech 在本地生成十六句不同回答；四个入口各有四句，通过洗牌队列保证一轮听完前不重复，并同步展示当前回答的日文和简体中文字幕。
8. 建立桌面、平板和手机尺寸的发布前浏览器清单，并对键盘操作、减少动态效果、音频失败降级、资源引用和 UTF-8 编码进行自动检查。
...
| `assets/v3/` | 首屏表情、换装与五秘密画桌等本地角色资源 |
| `assets/v4/` | 七张统一 4:3 的运行时画册 WebP 与七张 480 × 360 缩略图 |
| `assets/audio/v4/` | 十六句不同回答的 MP3、完整 `audio_query` 与机器可读合成清单 |
...
| `docs/gallery-v4-image-prompts.md` | 四张新增害羞画册场景的完整提示词、参考图与导出规格 |
...
5. 画廊在原有“深夜专注、躲在画册后、抱玩偶晚安”之外，补充“床上画画、举起画稿、躲在靠枕后递画、装作没等夸奖”四种情景；运行时按七幕小故事排列，不再用换装图填充画廊。
6. 五秘密画桌以首屏身份图、深夜画稿图和家居服图作为参考，明确固定耳机、轻小说稿件、数位板、猫咪玩偶和带锁抽屉五个互不遮挡的点击区域。
7. 生成时明确要求全年龄、非性感化、无图片内文字、无水印、不复刻官方截图。

旧版素材保留在仓库中作为回退；首屏与换装继续使用 `assets/v3/`，运行时画册使用 `assets/v4/`。新增场景的完整提示词与裁切方法见 `docs/gallery-v4-image-prompts.md`。
...
四类互动分别是房门口、画桌旁、被夸以后与晚安以前，每类包含四句独立日文文本。写法遵循“轻声、短句、先警戒或嘴硬、句尾才稍微软下来”的节奏；涉及画稿时语气更认真，被夸时会先确认是不是客套。它表达的是本页角色设定与情境，不照录动画台词，也不冒充真实声优。

运行时数据保存在 `script.js` 的 `CONTENT.voiceLines`。`chooseVoiceReply()` 为每个入口维护一份洗牌队列：四句全部弹出前不会重复；重新洗牌时若队首会与上一轮末句相同，会先交换位置。字幕、表情、反应文字和音频路径都来自同一个回答对象，因此不会出现“声音换了但字幕没换”的错配。
...
- 根据句意使用同一模型提供的 `ささやき`、`ノーマル`、`じょうきげん` 或 `ふきげん` 表达；`pitchScale` 始终为 0，没有用后期移调制造不同音色。
...
旧版的四个原始文件为 44.1 kHz、单声道 WAV。v3 页面曾从该场景的原始 WAV 和两段后期 MP3 中随机选择；当前 v4 页面已经改为十六句独立回答。开始新回答前仍会停止上一段；播放失败时保留日文、简体中文字幕和视觉反馈，且不会回退到系统 TTS。

以上记录了旧版 WAV 校验值；v4 已为全部十六句保存完整 `audio_query`、引擎版本和机器可读清单。
...
2026-08-11 本轮更新完成 JavaScript 语法、CSS 解析、本地资源引用、十六段音频解码、UTF-8、热点几何和核心数据检查；五个热点在最窄画桌宽度下仍保持至少 44px 且互不重叠，四个语音入口各有四句不同回答，一轮听完前不重复，音频失败时字幕仍保留。受控开发容器禁止 Chromium 创建系统套接字，因此本轮没有在本地重新生成多视口截图；发布前仍须在可启动浏览器的环境中复核下列完整清单：

- 1440×900、1024×768 和 390×844 三种视口的布局与横向溢出检查。
- 门板键盘操作、三套换装、七张画廊内容、五个彩蛋、隐藏留言和 v1 状态迁移。
- 四个入口、十六句回答的洗牌轮换、连续播放、跨场景打断和缺失文件降级。
...
- 当前 `assets/v3/` 十一张 PNG 仍接近 20 MB；五秘密画桌已提供约 115 KB 的 WebP 优先资源与 PNG 回退，其余人物图仍需继续生成 WebP 或 AVIF，并为缩略图和移动端提供更小尺寸。
