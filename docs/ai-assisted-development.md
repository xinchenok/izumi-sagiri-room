...
- 当前用户可见功能基线：[`27f80fc`](https://github.com/xinchenok/izumi-sagiri-room/commit/27f80fcf2c8dc82be9a2aed34186775cc22a7bcb)
- 当前页面缓存标识：`20260811-5`
...
本节是项目截至 2026-08-11 的交付账本。它记录已经进入 GitHub `main` 并由 GitHub Pages 提供的实际工作；未完成的规划仍只放在“已知问题与后续规划”，不会混写成已交付功能。
...
- 将约 1.5 MB 的标题 TTF 缩成约 80 KB 的当前页面字符子集 WOFF2；画册和衣橱由全量预取改为仅预取当前相邻内容。
- 在同一 390px 冷缓存条件下，页面请求量从约 10.79 MB 降到约 1.04 MB，减少约 90.4%；运行时不热链第三方图片、字体或音频。
- 将持久状态集中到 `sagiri-room-state-v2`，保存服装、五秘密、回访、画册位置、音量和只看字幕偏好，并保留从 v1 迁移服装与秘密记录的兼容逻辑。
...
- 分别保存五秘密画桌、八张新增害羞画册和两套新增换装的提示词、参考素材、导出规格与校验值，方便后续在不破坏角色一致性的前提下维护。
...
- 正式网址返回 `HTTP 200`，线上 HTML 使用 `styles.css?v=20260811-5` 与 `script.js?v=20260811-5`；线上 HTML、CSS 和 JavaScript 与功能基线逐字节一致。
