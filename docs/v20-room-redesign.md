...
- 发布状态：本地 57 项测试全部通过，含真实音频与 `file://`；独立审查四项视觉修复均已解决，工具逐像素流程 F1 仍未闭环。detector 唯一一次执行有 warnings 且捕获截断，不是全绿。CI、合并、Pages 和正式网址复核仍待最终凭证。
- 发布状态唯一凭证入口：`docs/v20-release-receipt.json`。在凭证形成前，代码、测试通过、部署成功和线上验证必须分开描述。
...
| 远端 CI、合并、GitHub Pages、正式网址及资源指纹 | pending |
| 文档收尾 | 随最终发布凭证继续核对 |
