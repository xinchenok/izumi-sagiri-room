...
> 历史素材记录：以下“当前／现役”均指文内日期对应的旧版本，不是 V20。当前分层人物、日夜背景与真实声音调度见 [V20 房间重构](v20-room-redesign.md)，旧制作细节原样保留。
...
> 当前状态（2026-08-20）：场景图与衣橱素材仍是现役；下文四段 `assets/audio/v7` 拟音只保留为 V7 历史复现记录，页面已经不再读取。当前房间使用十八条专属角色语音、十二份 V14 真实事件拟音和两段天气声，权威说明及机器可读清单见 [`room-audio-v8.md`](room-audio-v8.md)。
...
维护时若替换新衣橱场景，应同步更新本表、`CONTENT.livingRoom`、`DESIGN.md` 和 `docs/ai-assisted-development.md`；若替换当前配音或场景声，应改 `docs/room-audio-v8.md` 与相应 V8 / V14 清单及来源合同。两类改动都要重新检查 1440、1024、390px、方向键、手机横滑、旧 V6 存档和减少动态效果。
